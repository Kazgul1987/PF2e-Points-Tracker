...
class BaseResearchTrackerApp extends HandlebarsApplicationMixin(ApplicationV2) {
  /**
...
export class PointsTrackerApp extends BaseResearchTrackerApp {
  ...
  async _prepareContext(options) {
    ...
    if (this.researchTracker) {
      researchData = await BaseResearchTrackerApp.prototype._prepareContext.call(this, options);
  ...
  _onRender(context, options) {
    if (this.researchTracker) super._onRender(context, options);
    else ApplicationV2.prototype._onRender.call(this, context, options);
    const html = globalThis.jQuery(this.element);
...
if (Hooks?.on) {
  ...
  Hooks.on("pf2ePointsTrackerUpdated", rerenderTrackers);
}
